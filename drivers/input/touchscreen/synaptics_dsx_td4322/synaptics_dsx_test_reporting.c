...
#define COMMAND_TIMEOUT_20MS 50
...
#define EXTEND_EE_SHORT_TEST_LIMIT_PART1  100
#define EXTEND_EE_SHORT_TEST_LIMIT_PART2  96         // ( unit = ratio )
...
/* tddi f54 production test cases + */
...
static void test_set_report_size(void)
{
	...
	case F54_FULL_RAW_PARTITION:
		...
		f54->report_size = 2 * tx * MAX_FINGER_PROTECT_RX;
		break;
	case F54_NOISE_TDDI:
		if (f55->extended_amp_btn) {
			tx += 1;
...
static int test_wait_for_command_completion(void)
{
	...
	do {
		...
		msleep(20);
		timeout_count++;
	} while (timeout_count < COMMAND_TIMEOUT_20MS);

	
	if (timeout_count == COMMAND_TIMEOUT_20MS) {
		dev_err(&rmi4_data->i2c_client->dev,
...
static ssize_t test_sysfs_tddi_full_raw_partition_test(struct device *dev,
		char *buf, int *size) {
	int retval = 0;
	unsigned int full_raw_report_size = *size;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	int count = 1;

	/* get the report image 92 */
	retval = test_sysfs_read_report(dev, NULL, "124", count,
				false, false);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to read report 92. exit\n", __func__);
	...
	retval = secure_memcpy(buf, full_raw_report_size,
		f54->report_data, f54->report_size, f54->report_size);
...
static ssize_t test_sysfs_tddi_full_raw_test(struct device *dev,
		char *buf, struct synaptics_rmi4_limit *limit_fw) {
	unsigned int i;
	unsigned int j;
	int count = 0;
	int tx_num = f54->tx_assigned;
	...
	unsigned short *limit_data_16;
	unsigned char buf_store[64];
	struct limit_header *header = limit_fw->header;
	...
	if (g_flag_readrt_err) {
		...
		return snprintf(buf, PAGE_SIZE, "ERROR: fail to read report image\n");
	}

	if (limit_fw == NULL) {
		return snprintf(buf, PAGE_SIZE, "ERROR: limit firmware is NULL\n");
	}


	report_data_16 = (unsigned short *)g_tddi_full_raw_data_output;
	limit_data_16 = (unsigned short *)(limit_fw->fw->data + header->array_limit_offset);

	for (i = 0; i < tx_num; i++) {
		for (j = 0; j < rx_num; j++) {
			if (limit_fw->fd >= 0) {
				sprintf(buf_store, "%d, ", *report_data_16);
				sys_write(limit_fw->fd, buf_store, strlen(buf_store));
			}

			if (count >= header->array_limit_size) {
				return snprintf(buf, PAGE_SIZE, "ERROR:Limit image out of range at %d\n", count);
			}
			if ((*report_data_16 < *(limit_data_16)) || (*report_data_16 > *(limit_data_16 + 1))) {
				return snprintf(buf, PAGE_SIZE, "ERROR:Failed at tx:%d rx:%d data:%d range[%d %d]\n", i, j, *report_data_16, *(limit_data_16), *(limit_data_16 + 1));
			}
			count += 4;
			report_data_16++;
			limit_data_16 += 2;
	...
	if (f55->extended_amp_btn) {
		for (i = 0; i < NUM_BUTTON; i++) {
			...
			if ((*report_data_16 < *(limit_data_16)) || (*report_data_16 > *(limit_data_16 + 1))) {
				return snprintf(buf, PAGE_SIZE, "ERROR:Failed at button %d range[%d %d]\n", i, *(limit_data_16), *(limit_data_16 + 1));
			}
	...
	return snprintf(buf, PAGE_SIZE, "PASS:BASELINE-COMPARE\n");
}
...
static ssize_t test_sysfs_tddi_delta_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval = 0;
	int tx_num = f54->tx_assigned;
	int rx_num = f54->rx_assigned;
	unsigned char *delta_raw;
	int cnt = 0, count = 0;
	int delta_report_size = 0;
	short *report_data_16;
	int i = 0, j = 0;
	struct synaptics_rmi4_data *rmi4_data = f54->rmi4_data;
	...
	delta_report_size  = tx_num * rx_num * 2;

	delta_raw = kzalloc(delta_report_size, GFP_KERNEL);
	if (delta_raw == NULL) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for g_tddi_full_raw_data_output\n",
				__func__);
		cnt = snprintf(buf + cnt, PAGE_SIZE, "%s\n", "No more memory\n");
		return -ENOMEM;
	}

	/* get the report image 92 */
	retval = test_sysfs_read_report(dev, attr, "2", 1,
				false, false);
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to read report 92. exit\n", __func__);
		cnt = snprintf(buf + cnt, PAGE_SIZE, "%s\n", "Get delta data failed\n");
		kfree(delta_raw);
		return -EIO;
	}

	secure_memcpy(delta_raw, delta_report_size,
		f54->report_data, f54->report_size, f54->report_size);

	report_data_16 = (short *)delta_raw;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (g_tddi_ee_short_data_error)
		kfree(g_tddi_ee_short_data_error);
	
	g_tddi_ee_short_data_error = kzalloc(PAGE_SIZE, GFP_KERNEL);

	if (!g_tddi_ee_short_data_error) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for g_tddi_ee_short_data_error\n",
				__func__);
		return -ENOMEM;
	}
	...
	control.reg_99->integration_duration_lsb = EXTEND_EE_SHORT_INT_DUR;
	control.reg_99->integration_duration_msb = (EXTEND_EE_SHORT_INT_DUR >> 8) & 0xff;
	control.reg_99->reset_duration = EXTEND_EE_SHORT_RESET_DUR;
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_99->address,
			control.reg_99->data,
			sizeof(control.reg_99->data));
	...
	control.reg_182->cbc_timing_ctrl_tx_lsb = EXTEND_EE_SHORT_TX_ON_COUNT;
	control.reg_182->cbc_timing_ctrl_tx_msb = (EXTEND_EE_SHORT_TX_ON_COUNT >> 8) & 0xff;
	control.reg_182->cbc_timing_ctrl_rx_lsb = EXTEND_EE_SHORT_RX_ON_COUNT;
	control.reg_182->cbc_timing_ctrl_rx_msb = (EXTEND_EE_SHORT_RX_ON_COUNT >> 8) & 0xff;
	retval = synaptics_rmi4_reg_write(rmi4_data,
			control.reg_182->address,
	...
	/* use the upper half as part 1 image */
	/* the data should be lower than TEST_LIMIT_PART1 ( fail, if > TEST_LIMIT_PART1 ) */
	for (i = 0, offset = 0; i < (tx_num -1 ) * rx_num; i++) {
		tddi_rt95_part_one[i] = (signed short)(f54->report_data[offset]) |
								((signed short)(f54->report_data[offset + 1]) << 8);
		offset += 2;
	}
	...
	for (i = 0; i < tx_num - 1; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (tddi_rt95_part_one[i*rx_num + j] > EXTEND_EE_SHORT_TEST_LIMIT_PART1) {
				dev_err(&f54->rmi4_data->i2c_client->dev,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d in part 1 image (limit = %d)\n",
						__func__, i, j, tddi_rt95_part_one[i*rx_num + j], EXTEND_EE_SHORT_TEST_LIMIT_PART1);
				if (err_count == 0)
					err_index = sprintf(g_tddi_ee_short_data_error + err_index, "part 1 image:(tx%-2d, rx%-2d) = %-4d ", i, j, tddi_rt95_part_one[i*rx_num + j]);
				err_count++;
				tddi_rt95_part_one[i*rx_num + j] = _TEST_FAIL; // 1: fail
			}
			else {
	...
	err_index = sprintf(g_tddi_ee_short_data_error + err_index, "count %d\n", err_count);
	err_count = 0;
	/* step 6 */
	/* use the lower half as part 2 image */
	/* and perform the calculation */
	/* the calculated data should be over than TEST_LIMIT_PART2 ( fail, if < TEST_LIMIT_PART2 ) */
	for (i = 0, offset = buffer_size; i < (tx_num - 1) * rx_num; i++) {
		tddi_rt95_part_two[i] = (signed short)(f54->report_data[offset]) |
								((signed short)(f54->report_data[offset + 1]) << 8);
		offset += 2;
	}
	...
	for (i = 0; i < tx_num - 1; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (tddi_rt95_part_two[i*rx_num + j] < EXTEND_EE_SHORT_TEST_LIMIT_PART2) {
				dev_err(&f54->rmi4_data->i2c_client->dev,
						"%s: fail at (tx%-2d, rx%-2d) = %-4d in part 2 image (limit = %d)\n",
						__func__, i, j, tddi_rt95_part_two[i*rx_num + j], EXTEND_EE_SHORT_TEST_LIMIT_PART2);
				if (err_count == 0)
					err_index = sprintf(g_tddi_ee_short_data_error + err_index, "part 2 image:(tx%-2d, rx%-2d) = %-4d ", i, j, tddi_rt95_part_two[i*rx_num + j]);
				err_count++;
				tddi_rt95_part_two[i*rx_num + j] = _TEST_FAIL; // 1: fail
			}
			else {
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (g_flag_readrt_err) {
		...
		kfree(g_tddi_ee_short_data_error);
		g_tddi_ee_short_data_error = NULL;
		return snprintf(buf, PAGE_SIZE, "ERROR: fail to read report image\n");
	}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* allocate the g_abs_0d_open_data_output */
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (g_tddi_amp_open_data_error)
		kfree(g_tddi_amp_open_data_error);
	g_tddi_amp_open_data_error = kzalloc(PAGE_SIZE, GFP_KERNEL);
	if (!g_tddi_amp_open_data_error) {
		dev_err(&rmi4_data->i2c_client->dev,
				"%s: Failed to alloc mem for g_tddi_amp_open_data_error\n",
				__func__);
		return -ENOMEM;
	}
	...
	for (i = 0; i < tx_num - 1; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (p_rt92_delta_image[i * rx_num + j] < ELEC_OPEN_TEST_LIMIT_ONE) {
				...
				if (err_count == 0) {
					err_index = sprintf(g_tddi_amp_open_data_error + err_index, "phase 1 delta:(tx%-2d, rx%-2d) = %-4d ", i, j, p_rt92_delta_image[i*rx_num + j]);
				}
				err_count++;
				p_rt92_image_1[i*rx_num + j] = _TEST_FAIL; // 1: fail
			}
	...
	for (i = 0; i < tx_num - 1; i++) {
		for (j = 0; j < rx_num; j++) {
			...
			if (p_rt92_delta_image[i * rx_num + j] < ELEC_OPEN_TEST_LIMIT_TWO) {
				...
				if (err_count == 0) {
					err_index = sprintf(g_tddi_amp_open_data_error + err_index, "phase 2 delta:(tx%-2d, rx%-2d) = %-4d ", i, j, p_rt92_delta_image[i*rx_num + j]);
				}
				err_count++;
				p_rt92_image_2[i*rx_num + j] = _TEST_FAIL; // 1: fail
			}
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (g_flag_readrt_err) {
		...
		kfree(g_tddi_amp_open_data_error);
		g_tddi_amp_open_data_error = NULL;
...
static ssize_t synaptics_rmi4_test_invoke(struct synaptics_rmi4_data *rmi4_data, enum exp_invoke_fn command, char *buf, void *data){
	ssize_t ret_size = 0;

	dev_info(&rmi4_data->i2c_client->dev, "%s: %d\n", __func__, command);
	switch(command) {
	case RMI_TDDI_EXTEND_EE_SHORT_STORE:
		ret_size = test_sysfs_tddi_extend_ee_short_store(&rmi4_data->i2c_client->dev, NULL, buf, 1);
		break;
	case RMI_TDDI_EXTEND_EE_SHORT_SHOW:
		ret_size = test_sysfs_tddi_extend_ee_short_show(&rmi4_data->i2c_client->dev, NULL, buf);
		break;
	case RMI_TDDI_FULL_RAW_STORE:
		ret_size = test_sysfs_tddi_full_raw_store(&rmi4_data->i2c_client->dev, NULL, "1", 1);
		break;
	case RMI_TDDI_FULL_RAW_SHOW:
		ret_size = test_sysfs_tddi_full_raw_show(&rmi4_data->i2c_client->dev, NULL, buf);
		break;
	case RMI_TDDI_FULL_RAW_TEST:
		ret_size = test_sysfs_tddi_full_raw_test(&rmi4_data->i2c_client->dev, buf, data);
		break;
	case RMI_TDDI_ELECTORDE_OPEN_STORE:
		ret_size = test_sysfs_tddi_amp_electrode_open_store(&rmi4_data->i2c_client->dev, NULL, "1", 1);
		break;
	case RMI_TDDI_ELECTORDE_OPEN_SHOW:
		ret_size = test_sysfs_tddi_amp_electrode_open_show(&rmi4_data->i2c_client->dev, NULL, buf);
		break;
	case RMI_TDDI_DELTA_SHOW:
		ret_size = test_sysfs_tddi_delta_show(&rmi4_data->i2c_client->dev, NULL, buf);
		break;
	case RMI_TDDI_FULL_RAW_PARTITION:
		ret_size = test_sysfs_tddi_full_raw_partition_test(&rmi4_data->i2c_client->dev, buf, (int *)data);
		break;
	default:
		dev_err(&rmi4_data->i2c_client->dev, "invalid command %d\n", command);
		break;
	}
...
static struct synaptics_rmi4_exp_fn test_module = {
	...
	.invoke = synaptics_rmi4_test_invoke,
};
