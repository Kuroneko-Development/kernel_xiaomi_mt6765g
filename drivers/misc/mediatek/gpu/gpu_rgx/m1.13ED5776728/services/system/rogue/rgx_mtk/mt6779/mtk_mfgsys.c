	...
	do { \
		if (clk) { \
			if (clk_prepare_enable(clk)) \
				no_printk("PVR_K: clk_prepare_enable " \
	...
	do { \
		if (clkC && clkP) { \
			if (clk_set_parent(clkC, clkP)) \
				no_printk("PVR_K: clk_set_parent " \
...
#ifdef MTK_DEBUG
	if (gpu_debug_enable)
		no_printk("PVR_K: 3DFreq=%d, Volt=%d\n",
...
PVRSRV_ERROR MTKMFGSystemInit(void)
{
	...
	if (!g_gpu_base)
		no_printk("@%s: g_gpu_base iomap failed", __func__);
	else
		no_printk("@%s: g_gpu_base: 0x%p", __func__, g_gpu_base);

	if (!g_mfgcfg_base)
		no_printk("@%s: g_mfgcfg_base iomap failed", __func__);
	else
		no_printk("@%s: g_mfgcfg_base: 0x%p", __func__, g_mfgcfg_base);
