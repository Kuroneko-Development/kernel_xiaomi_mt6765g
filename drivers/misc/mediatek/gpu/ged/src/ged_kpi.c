	...
	, GED_TIMESTAMP_TYPE eTimeStampType)
{
	...
	if (unlikely(miss_tag_head == NULL)) {
		...
		if (miss_tag_head) {
			...
			GED_no_printk("[GED_KPI][Exception]");
			GED_no_printk(
	...
	case GED_TIMESTAMP_TYPE_D:
...
#ifdef GED_KPI_DEBUG
			GED_no_printk("[GED_KPI][Exception]");
			GED_no_printk("no hashtable head for ulID: %lu\n", ulID);
#endif
		}
		...
		if (!psHead) {
			...
			if (psHead) {
				...
				GED_no_printk("(sizeof(GED_KPI_HEAD)) failed\n");
				goto work_cb_end;
			}
		}
		memset(psKPI, 0, sizeof(GED_KPI));
	...
	void *fence_addr)
{
	...
	if (g_psWorkQueue && is_GED_KPI_enabled) {
		...
		if (!psTimeStamp) {
			GED_no_printk("[GED_KPI]: GED_ERROR_OOM in %s\n",
				__func__);
