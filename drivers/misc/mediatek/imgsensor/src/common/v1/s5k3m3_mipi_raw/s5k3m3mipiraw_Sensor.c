 ...
 *
 *	//no_printk("frame_length = %d ", frame_length);
 *
 *}
 */
	...
	kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	no_printk("shutter =%d, framelength =%d/%d, dummy_line=%d\n",
...
{
	no_printk("E, version_id:%d (0:orig, 1:rom update)\n", version_id);
...
{
	no_printk("E! currefps:%d, version_id:%d (0:orig, 1:rom update)\n",
		currefps, version_id);
	  ...
	  (read_cmos_sensor_byte(0x0002) << 8) | read_cmos_sensor_byte(0x0003);
		...
		"i2c write id: 0x%x, ReadOut sensor id: 0x%x, sensor_id:0x%x Module id: 0x%x.\n",
				...
				if (module_id == S5K3M3_MODULE_ID_V1)
					...
					no_printk("Module id: 0x%x unsupported\n",
						module_id);
					return ERROR_INVALID_DRIVER_MOD_ID;
				}

				no_printk("Module id: 0x%x, version_id: %d\n",
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		no_printk("capture30fps: use cap30FPS's setting: %d fps!\n",
		 ...
		 */
		no_printk("cap115fps: use cap1's setting: %d fps!\n",
				  ...
				  UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		no_printk("SENSOR_FEATURE_GET_PDAF_INFO scenarioId:%llu\n",
			*feature_data);
