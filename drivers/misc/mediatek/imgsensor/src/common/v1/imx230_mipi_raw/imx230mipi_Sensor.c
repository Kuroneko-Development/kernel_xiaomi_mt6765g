...
static void imx230_set_pd_focus_area(MUINT32 startpos, MUINT32 size)
{
	...
	if ((cur_startpos == startpos) && (cur_size == size)) {
		no_printk("Not to need update focus area!\n");
		return;
	}
...
static void imx230_get_pdaf_reg_setting(MUINT32 regNum, kal_uint16 *regDa)
{
	...
	for (i = 0; i < regNum; i++) {
		...
		/* no_printk("%x %x", regDa[idx], regDa[idx+1]); */
	}
}
...
static void set_shutter(kal_uint32 shutter)
{
	...
	no_printk("Enter! shutter =%d, framelength =%d\n",
		shutter,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	no_printk("[IMX230MIPI]enter IMX230MIPIGain2Reg function\n");
	...
	if (iI != IMX230MIPI_MaxGainIndex) {
		if (gain != IMX230MIPI_sensorGainMapping[iI][0]) {
			no_printk("Gain mapping don't correctly:%d %d\n", gain,
				IMX230MIPI_sensorGainMapping[iI][0]);
		}
	}
	no_printk("exit IMX230MIPIGain2Reg function\n");
	...
	kal_uint16 le, kal_uint16 se, kal_uint16 gain)
{
	...
	no_printk("le:0x%x, se:0x%x, gain:0x%x\n", le, se, gain);
	spin_lock(&imgsensor_drv_lock);
...
static kal_uint32 imx230_awb_gain(struct SET_SENSOR_AWB_GAIN *pSetSensorAWB)
{
	...
	no_printk("[imx230_awb_gain] ABS_GAIN_GR:%d, grgain_32:%d\n",
		pSetSensorAWB->ABS_GAIN_GR,
		grgain_32);
	no_printk("[imx230_awb_gain] ABS_GAIN_R:%d, rgain_32:%d\n",
		pSetSensorAWB->ABS_GAIN_R,
		rgain_32);
	no_printk("[imx230_awb_gain] ABS_GAIN_B:%d, bgain_32:%d\n",
		pSetSensorAWB->ABS_GAIN_B,
		bgain_32);
	no_printk("[imx230_awb_gain] ABS_GAIN_GB:%d, gbgain_32:%d\n",
 ...
 */
	...
	case SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY:
		no_printk("SENSOR_FEATURE_GET_SENSOR_HDR_CAPACITY scenarioId:%llu\n",
			*feature_data);
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		no_printk("SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%llu\n",
			*feature_data);
	...
	case SENSOR_FEATURE_GET_PDAF_REG_SETTING:
		no_printk("SENSOR_FEATURE_GET_PDAF_REG_SETTING %d",
			(*feature_para_len));
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
		/*no_printk("SENSOR_FEATURE_SET_PDAF_REG_SETTING %d",
		 *	(*feature_para_len));
		 */
