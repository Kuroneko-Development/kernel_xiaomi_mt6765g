			...
			kal_uint16 frame_length)
{
	...
	write_cmos_sensor(0x0203, shutter  & 0xFF);
	//write_cmos_sensor(0x0104, 0x00);
	no_printk("Add for N3D! shutter =%d, framelength =%d\n",
		shutter, imgsensor.frame_length);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	no_printk("S5K5E8YXFRONT_I2CBUS = %d\n", I2C_bus);
	if (I2C_bus != S5K5E8YXFRONT_I2CBUS) {
		*sensor_id = 0xFFFFFFFF;
		no_printk("S5K5E8YXFRONT_I2CBUS:%d Check Error!\n", I2C_bus);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			no_printk("Read id fail,i2c_write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			no_printk("R sensorid fail,write_id 0x%x id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap.max_framerate) {
		...
		no_printk("current_fps %d not support, so cap1's: %d fps!\n",
		...
		no_printk("current_fps %d not support, so use cap2's: %d fps!\n",
			imgsensor.current_fps,
			...
			MUINT32 framerate)
{
	...
	case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
		...
		if (imgsensor.frame_length > imgsensor.shutter)
			...
			/*No need to set*/
			no_printk("frame_length %d < shutter %d",
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		if (imgsensor.current_fps ==
			...
			if (imgsensor.current_fps !=
				...
				no_printk("%d fps not support,cap's: %d fps\n",
					framerate,
	...
	default:  //coding with  preview scenario by default
		...
		no_printk("error scenario_id = %d, preview scenario\n",
			scenario_id);
		break;
	}
	return ERROR_NONE;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	/*no_printk("feature_id = %d", feature_id);*/
	switch (feature_id) {
	...
	case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
		no_printk("imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
	...
	case SENSOR_FEATURE_SET_HDR:
		...
		no_printk("Warning! Not Support IHDR Feature");
		spin_lock(&imgsensor_drv_lock);
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		no_printk("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%llu\n",
			*feature_data);
	...
	case SENSOR_FEATURE_SET_IHDR_SHUTTER_GAIN:
		no_printk("SET_SENSOR_IHDR LE=%d, SE=%d, Gain=%d\n",
