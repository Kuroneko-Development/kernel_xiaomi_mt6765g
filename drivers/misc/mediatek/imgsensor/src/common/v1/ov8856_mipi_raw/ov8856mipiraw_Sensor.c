...
static void set_dummy(void)
{
	...
	no_printk("imgsensor.frame_length = %d\n", imgsensor.frame_length);
...
static void set_max_framerate(UINT16 framerate, kal_bool min_framelength_en)
{
	...
	no_printk("framerate = %d, min framelength should enable?\n", framerate);
				...
				kal_uint16 shutter, kal_uint16 frame_length)
{
	...
	no_printk("Add for N3D! shutterlzl =%d, framelength =%d\n",
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
		   ...
		   (read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			no_printk("Read sensor id fail,write_id:0x%x, id: 0x%x\n",
...
/*const kal_uint8 i2c_addr[] = {IMGSENSOR_WRITE_ID_1, IMGSENSOR_WRITE_ID_2};*/
	...
	no_printk("PLATFORM:Vison,MIPI 4LANE\n");
	no_printk("read_cmos_sensor(0x302A): 0x%x\n", read_cmos_sensor(0x302A));
	/* sensor have two i2c address 0x6c 0x6d & 0x21 0x20,
	 * we should detect the module used i2c address
	 */
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
	...
	mdelay(10);
	#ifdef OV8856R1AOTP
	/*	no_printk("Apply the sensor OTP\n");
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_TEMPERATURE_VALUE:
		no_printk("This sensor can't support temperature get\n");
		break;
