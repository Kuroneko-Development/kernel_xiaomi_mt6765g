...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	#endif
		no_printk("Read ok sensor id: 0x%x, img sensor_id:0x%x.\n",
			*sensor_id, imgsensor_info.sensor_id);
		return ERROR_NONE;
	}
	no_printk("Readsensoridfail: 0x%x, img_info.sensor_id:0x%x.\n",
		*sensor_id, imgsensor_info.sensor_id);
		retry--;
	} while (retry > 0);
	i++;
...
static kal_uint32 open(void)
{
	...
	if (sensor_id == imgsensor_info.sensor_id) {
		...
		no_printk("Read sensor id fail, id: 0x%x, sensor id: 0x%x\n",
			imgsensor.i2c_write_id, sensor_id);
		retry--;
	} while (retry > 0);
	i++;
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	for (i = 0; i < 10; i++) {
		no_printk("delay time = %d, the frame no = %d\n",
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		no_printk("current_fps %d not support,use cap1\n",
				imgsensor.current_fps/10);
		imgsensor.pclk = imgsensor_info.cap1.pclk;
		imgsensor.line_length = imgsensor_info.cap1.linelength;
		imgsensor.frame_length = imgsensor_info.cap1.framelength;
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
	    no_printk("GET_CROP_INFO scenarioId:%d\n",
			(UINT32)*feature_data);
	...
	case SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY:
		no_printk("PDAF_CAPACITY scenarioId:%llu\n", *feature_data);
		switch (*feature_data) {
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		no_printk("GET_PDAF_INFO scenarioId:%llu\n", *feature_data);
