...
static void write_shutter(kal_uint16 shutter)
{
	...
	cam_no_printk("shutter =%d, framelength =%d, realtime_fps =%d\n",
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	cam_no_printk("gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
{
	cam_no_printk("E RES_2112x1568_30fps\n");
#if MULTI_WRITE
	ov13b10_table_write_cmos_sensor(
...
static void capture_setting(kal_uint16 currefps)
{
	...
	cam_no_printk("E 4224x3136_zsl_30fps currefps = %d\n",
		currefps);
#if MULTI_WRITE
	if (currefps == 150) {
		ov13b10_table_write_cmos_sensor(
...
{
	cam_no_printk("E RES_4224x3136_zsl_30fps\n");
#if MULTI_WRITE
	ov13b10_table_write_cmos_sensor(
...
{
	cam_no_printk("E RES_640x480_120fps\n");
#if MULTI_WRITE
	ov13b10_table_write_cmos_sensor(
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		cam_no_printk("imgsensor id: 0x%x fail\n", *sensor_id);
...
static kal_uint32 open(void)
{
	...
	if (imgsensor_info.sensor_id != sensor_id) {
		cam_no_printk("Open sensor id: 0x%x fail\n", sensor_id);
		return ERROR_SENSOR_CONNECT_FAIL;
	}
			  ...
			  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			cam_no_printk("current_fps %d fps is not support,use cap1: %d fps!\n",
...
static kal_uint32 get_default_framerate_by_scenario(
			...
			MUINT32 *framerate)
{
	cam_no_printk("[3058]scenario_id = %d\n", scenario_id);
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		cam_no_printk("GET_CROP_INFO scenarioId:%d\n",
			*feature_data_32);
