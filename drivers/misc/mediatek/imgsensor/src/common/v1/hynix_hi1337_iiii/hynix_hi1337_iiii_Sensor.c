...
static int hi1337_vendor_id_read(int addr)
{
	...
	no_printk("hynix_hi1337_IIII read vendor id from 0x%x is: 0x%x\n", addr, val);
	return val;
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    if( flag != HI1337_VENDOR_ID) {
        no_printk("hynix_hi1337_IIII match vendor id fail, read vendor id is: 0x%x, expect vendor id is 0x%x \n", flag, HI1337_VENDOR_ID);
        return ERROR_SENSOR_CONNECT_FAIL;
    } else {
        hi1337_fusion_id_read();
    }
    no_printk("hynix_hi1337_IIII match vendor id success, read vendor id is: 0x%x, expect vendor id is 0x%x\n", flag, HI1337_VENDOR_ID);
