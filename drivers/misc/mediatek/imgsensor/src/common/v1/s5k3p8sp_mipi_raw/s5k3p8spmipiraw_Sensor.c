...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		no_printk("read_0x0000=0x%x, 0x0001=0x%x,	0x0000_0001=0x%x\n",
			...
			if (is_RWB_sensor() == 0x1) {
				...
				no_printk("RWB sensor of S5k3p8\n");
				}
#endif
			sp8spFlag = (((read_cmos_sensor(0x000C) & 0xFF) << 8)
				    |((read_cmos_sensor(0x000E) >> 8) & 0xFF));
			no_printk("sp8Flag(0x%x),0x5003 used by s5k3p8sp\n",
				sp8spFlag);

			if (sp8spFlag == 0x5003) {
				no_printk("it is s5k3p8sp\n");
				return ERROR_NONE;
			}
		no_printk("3p8 type is 0x(%x),0x000C(0x%x),0x000E(0x%x)\n",
			sp8spFlag,
...
no_printk("feature_Control imgsensor.pclk = %d,imgsensor.current_fps = %d\n",
	 ...
	 * no_printk("SENSOR_FEATURE_GET_SENSOR_PDAF_CAPACITY scenarioId:%d\n",
	 * (UINT16)*feature_data);
