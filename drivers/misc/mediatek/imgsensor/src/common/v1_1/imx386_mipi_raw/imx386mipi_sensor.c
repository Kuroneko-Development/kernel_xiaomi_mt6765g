				...
				kal_uint32 addr, u8 *reg_buf, kal_uint32 size)
{
	...
	for (i = 0; i < size; i += MAX_READ_WRITE_SIZE) {
		...
		if (ret < 0)
			no_printk("write burst reg into sensor failed!\n");
...
static int imx386_read_otp(u16 addr, u8 *buf)
{
	...
	if (ret < 0)
		no_printk("read data from imx386 primax otp e2prom failed!\n");
...
static void load_imx386_spc_data(void)
{
	...
	0x7D4C + i, imx386_primax_otp_buf[spc_start_addr - OTP_START_ADDR + i]);
		...
		no_printk("SPC_Data[0x%x] = 0x%x\n",
			0x7D4C + i, read_cmos_sensor(0x7D4C + i));

		no_printk("OTP[0x%x] = 0x%x\n", spc_start_addr + i,
	...
	for (i = 0; i < OTP_DATA_SIZE; i++) {
		no_printk("========imx386_otp idx:0x%4x val:0x%x======\n", i,
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	no_printk("exit IMX386MIPI_sensorGainMapping function\n");
...
{
	no_printk("streaming_enable(0=Sw tandby,1=streaming): %d\n", enable);
	if (enable) {
		write_cmos_sensor(0x0100, 0X01);
